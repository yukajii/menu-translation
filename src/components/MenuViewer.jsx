    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between mb-4">
            <Link to="/" className="text-gray-500 hover:text-gray-700">← Back to MenuTap</Link>
            <h1 className="text-2xl font-bold text-gray-900">{menuData.restaurant}</h1>
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-colors ${
                  !showTranslation ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-900 hover:bg-gray-200'
                }`}
              >
                <ImageIcon size={20} />
                Original Menu
      ...
      <main className="flex-grow">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          {/* Original menu view */}
          {!showTranslation && (
            <div className="bg-white rounded-lg p-4 flex items-center justify-center min-h-[calc(100vh-240px)]">
              <img
                src={`/data/${restaurantId}/${
                  menuData.pages?.[currentPage - 1]?.image || 'menu.jpg'
                }`}
                alt={`Original menu - page ${currentPage}`}
                className="max-h-[calc(100vh-280px)] w-auto max-w-full rounded shadow-lg"
              />
            </div>
          )}
